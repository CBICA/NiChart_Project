...
def run_workflow(root_dir: Any, dict_config: Any) -> None:
    ...
    def step_spare():
        ...
        dict_var = "Code"
        covars = "MRID,Age,Sex,MUSE_702"
        utilw.select_vars(f_comb, dict_csv, dict_var, covars, f_spare_input)
    ...
    def step_centiles():
        ...
        roi_pref = "MUSE"
        suffix = "NONE"
        utilw.corr_icv(f_raw, f_raw, 'normICV', icv_var, roi_pref, suffix, f_normicv)
        
        # Merge covars to normalized ROIs
        key_var = "MRID"
        utilw.merge_dataframes(input_demog, f_normicv, key_var, f_normicv_wcover)
        ...
        utilw.calc_subject_centiles(f_normicv_wcover, cent_csv, list_rois, f_centiles)

    def step_combine():
        utilw.combine_demog_hroi_ml_cent(
            f_results, [input_demog, rois_sel, f_raw, f_spares, f_surrealgan]
        )
    ...
    f_normicv = os.path.join(out_dir, f"{dset_name}_normICV.csv")
    f_normicv_wcover = os.path.join(out_dir, f"{dset_name}_normICV_withcovar.csv")
    f_centiles = os.path.join(out_dir, f"{dset_name}_normICV_centiles.csv")
