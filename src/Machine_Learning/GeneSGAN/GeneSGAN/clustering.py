...
class Gene_SGAN_train():
    
    def __init__(self, ncluster, start_saving_epoch, max_epoch, WD_threshold, AQ_threshold, \
        cluster_loss_threshold, lam=9, mu=5, kappa = 0.1, batchSize=25, lipschitz_k = 0.5, \
        beta1 = 0.5, lr = 0.0002, genelr = 0.00002, max_gnorm = 100, eval_freq = 25, save_epoch_freq = 5, print_freq = 1000):
        self.opt=dotdict({})
    ...
    def parse_data(self, imaging_data, gene_data, covariate, random_seed, data_fraction):
        cn_train_dataset, pt_train_dataset, pair_train_dataset, correction_variables, normalization_variables = parse_train_data(imaging_data, gene_data, covariate, random_seed, data_fraction, self.opt.batchsize)
        cn_eval_dataset, pt_eval_dataset, gene_eval_dataset, __, __ = parse_validation_data(imaging_data, gene_data, covariate, random_seed, data_fraction, correction_variables, normalization_variables)
        self.opt.nROI = pt_eval_dataset.shape[1]
        self.opt.n_val_data = pt_eval_dataset.shape[0]
        self.opt.nGene = gene_eval_dataset.shape[1]
        return cn_train_dataset, pt_train_dataset, pair_train_dataset, cn_eval_dataset, pt_eval_dataset, gene_eval_dataset, correction_variables, normalization_variables


    def train(self, model_name, imaging_data, gene_data, covariate, save_dir, random_seed=0, data_fraction=1, verbose=True):
        if verbose: result_f = open("%s/results.txt" % save_dir, 'w')

        cn_train_dataset, pt_train_dataset, pair_train_dataset, eval_X, eval_Y, eval_gene, correction_variables, normalization_variables = self.parse_data(imaging_data, gene_data, covariate, random_seed, data_fraction)
        ...
        mean_distance_list = [0,0,0]                    ##### number of consecutive epochs with max_wd < threshold
        aq_loss_cluster_list = [[0 for _ in range(3)] for _ in range(2)]                    ##### number of consecutive epochs with aq and cluster_loss < threshold
        ...
        for epoch in range(1, self.opt.max_epoch + 1):
            ...
            for i, pt_data in enumerate(pt_train_dataset):
                ...
                pair_data = pair_train_dataset.next()
                real_X, real_Y = Variable(cn_data['x']), Variable(pt_data['y'])
                pair_gene, pair_gene_filled, pair_Y = Variable(pair_data['gene']), Variable(pair_data['gene_filled']), Variable(pair_data['y'])

                total_steps += self.opt.batchsize
                epoch_iter += self.opt.batchsize
                losses = model.train_instance(real_X, real_Y, pair_gene, pair_gene_filled, pair_Y, self.opt.kappa)
            ...
            if epoch % self.opt.eval_freq == 0:
                ...
                mean_distance, w_distances = eval_w_distances(eval_X, eval_Y, model)
                mean_distance_list.append(mean_distance)
                mean_distance_list.pop(0)
                ...
                t = time.time() - t
                if verbose:
                    res_str_list = ["[%d], Mean_W_Distance: %.4f, TIME: %.4f" % (epoch,mean_distance, t)]
                ...
                if len(w_distances)==model.opt.ncluster:
                    if max(mean_distance_list) < self.opt.WD_threshold and max(aq_loss_cluster_list[0]) < self.opt.AQ_threshold\
