...
with st.expander('Select dicom series', expanded=False):
    ...
    if len(st.session_state.sel_series) > 0:
        if st.session_state.paths['nifti'] != '':
            st.session_state.paths[st.session_state.sel_mod] = os.path.join(st.session_state.paths['nifti'], st.session_state.sel_mod)
    ...
    if btn_convert:
        with st.spinner('Wait for it...'):
            ...
            st.session_state.list_input_nifti = [f for f in os.listdir(st.session_state.paths[st.session_state.sel_mod]) if f.endswith('nii.gz')]
...
with st.expander('View images', expanded=False):
    ...
    if sel_img is not None:
        st.session_state.paths['sel_img'] = os.path.join(st.session_state.paths[st.session_state.sel_mod], sel_img)

    flag_btn = os.path.exists(st.session_state.paths['sel_img'])

    # Create a list of checkbox options
    list_orient = st.multiselect("Select viewing planes:", utilni.VIEWS, utilni.VIEWS, disabled = not flag_btn)
