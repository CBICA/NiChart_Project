...
def panel_upload_multi_subject():
 ...
 """
    ...
    if dtype == 'Nifti (folder)':
        ...
        ftype = ['.csv']
        csvupload_cols = st.columns(4)
        with csvupload_cols[0]:
            if st.button("Need a template?"):
                st.toast("Downloading your template CSV file...")
                try:
                    autogenerated_csv = generate_template_csv()
                    autogenerated_csv_data = autogenerated_csv.to_csv(index=False).encode('utf-8')
                    st.download_button(
                    ...
                    st.toast("We couldn't automatically generate a CSV for you. Do you have scans uploaded?")
    elif dtype == 'BIDS (folder)':
        accept_multiple_files = 'directory'
