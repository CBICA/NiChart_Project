...
def panel_upload_multi_subject():
        ...
        with st.popover("❓", width='content'):
            st.write(
                ...
                **Data Upload Guide**
                ...
                - You may upload MRI scans in **NIfTI** format:
                  - Selecting multiple .nii or .nii.gz files, or
                  - A single .zip file with multiple .nii.gz files
                ...
                - A participant csv with at least the MRID column is required! 
                
                - Nifti scans should be named consistently, as:  {MRID}_common_suffix.nii.gz
                
                - Once uploaded, NiChart will automatically:
                  - Organize the files into the standard input structure
                  - Verify consistency of participant list with scans
                  
                - You may open and edit the subject list
                
                - You can also upload additional non-imaging data (e.g., clinical or cognitive measures) as a CSV file.
    ...
    # Upload data
    sel_opt = sac.chip(
        ['Single (.zip, .csv)', 'Multiple (dicom files)'],
