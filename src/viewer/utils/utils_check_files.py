...
ftypes = [
    'unified', 'mapped', 'visits', 'imputed_in', 'imputed_out', 'merged', 'filtered', 'final'
]
fdirs = [
    'workingdir', 'workingdir', 'workingdir', 'workingdir', 'workingdir', 'workingdir', 'workingdir', ''
]
fprefs = [
...
df_files = pd.DataFrame(
    {'ftype':ftypes, 'fdir':fdirs, 'fpref':fprefs}
)
#####################################################################
...
def get_file_names(std, ftype):
    ...
    dout = os.path.join(
        st.session_state.paths['outdir'], std, df_sel.fdir.values[0]
    )
